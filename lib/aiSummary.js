...
async function generateAISummary({ title, url, content }) {
  ...
  const system = [
    ...
    "Do not invent, infer, or normalise calendar dates or months; if the source uses relative time (e.g. “next month”, “later this year”), preserve that phrasing exactly unless an explicit date is stated in the source.",
  ].join(" ");
